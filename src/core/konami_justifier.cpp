...
{
  m_irq_event =
    TimingEvents::CreateTimingEvent("Konami Justifier IRQ", 1, 1, [](void* param, TickCount ticks, TickCount ticks_late) { static_cast<KonamiJustifier*>(param)->IRQEvent(); }, this, false);
}
...
{
  return ControllerType::KonamiJustifier;
}
...
bool KonamiJustifier::DoState(StateWrapper& sw, bool apply_input_state)
{
...
void KonamiJustifier::UpdatePosition()
{
  ...
  if (mouse_x < 0 || mouse_y < 0 ||
      !g_gpu->ConvertScreenCoordinatesToBeamTicksAndLines(mouse_x, mouse_y, m_x_scale, &m_position_tick, &m_position_line))
  {
  ...
  else
  {
    ...
    m_irq_start_line = std::max<u32>((m_position_line >= 15) ? (m_position_line - 15) : 0, g_gpu->GetCRTCActiveStartLine());
    m_irq_end_line = std::min<u32>((m_position_line >= 10) ? (m_position_line - 10) : 0, g_gpu->GetCRTCActiveEndLine());

    if (m_position_tick > 150)
      m_position_tick -= 150;
...
void KonamiJustifier::UpdateIRQEvent()
{
  ...
  u32 target_line;
  if (current_line < m_irq_start_line || current_line > m_irq_end_line)
...
#if 0
  u32 tick, line;
  g_gpu->GetBeamPosition(&tick, &line);
  u32 xpos = tick / g_gpu->GetCRTCDotClockDivider();
  Log_WarningPrintf("IRQ @ %u/%u,%u (%u,%u)", tick, xpos, line, m_position_tick, m_position_line);
#endif
...
bool KonamiJustifier::GetSoftwareCursor(const Common::RGBA8Image** image, float* image_scale, bool* relative_mode)
{
  if (!m_crosshair_image.IsValid())
    return false;
